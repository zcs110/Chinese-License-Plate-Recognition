...
// crnn使用
extern std::vector<std::string> plate_chars;
...
-(void)textRecognition{
    ...
    self.inference = new Yolov5;
    self.inference->load(640);
    // 读取图像
    NSString *path_1 = [self getPathFromBundle:@"5.jpg"];
    UIImage *imagea = [UIImage imageWithContentsOfFile:path_1];
    cv::String filename = [path_1 UTF8String];
    UIImage *temp_image = [Utility scaleAndRotateImageBackCamera:imagea];
    cv::Mat image = [Utility cvMatFromUIImage:temp_image];
    __block std::vector<Object> resultObject;
    std::string plateStr;
    std::string plateColor;

    resultObject = self.inference->detectWithMat(image,plateStr,plateColor);
    std::vector<std::string> classNames;
    self.imageView.image = self.inference->draw(imagea, classNames, resultObject);
